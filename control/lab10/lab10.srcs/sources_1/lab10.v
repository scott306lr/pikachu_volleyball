...
//reg [17:0] x_addr[1:5][0:3];   // Address array for up to 8 fish images.
reg [17:0] y_addr[0:3];   // Address array for up to 8 fish images.
...
always @(posedge clk) begin
        if (~reset_n) begin
            ...
            x_clock <= (x_clock[32:21] >= 2 * (VBUF_W + FISH1_W) - 1)? 0 : x_clock + 1;
            y_clock <= (y_clock[31:20] >= 2 * (VBUF_H + FISH1_H) - 1)? 0 : y_clock + 1;
        end
end
...
always @(*) begin
  ...
  else if (chcolor && ~body[1] && ~body[2] && ~body[3]  )
    rgb_next = ~color_out;
  else
